...
const searchMusic = function (query) {
  return _call(loadFileList, function (fileList) {
    const order = getNameOrder();
    const library = fileList.map(f => toMusicItem(f, order));
    const q = (query || "").toLowerCase();
    const data = q ? library.filter(it => it.title.toLowerCase().includes(q) || it.artist.toLowerCase().includes(q)) : library;
    return {
      isEnd: true,
      data
    };
...
const buildSheetMusicList = _async(function (client, sheetId) {
  const roots = sheetId === ALL_SHEET_ID ? getSearchPathList() : [sheetId];
  const order = getNameOrder();
  const groups = [];
  return _continue(_forOf(roots, function (root) {
    const _push3 = groups.push;
    return _await(scanFolder(client, root), function (_scanFolder4) {
      _push3.call(groups, _scanFolder4);
    });
  }), function () {
...
const loadLyric = _async(function (musicItem) {
  let _exit2 = false;
  ...
  return _continue(_forOf(candidates, function (lrcPath) {
    return _catch(function () {
      return _await(fetchBytes(lrcPath, auth), function (bytes) {
        return function () {
          if (bytes && bytes.length) {
            return _await(decodeLyricBytes(bytes), function (text) {
              if (text) {
                cache[id] = text;
                const _rawLrc = {
                  rawLrc: text
...
const fetchBytes = _async(function (lrcPath, auth) {
  return _await(axios.get(DAV_BASE + encodeURI(lrcPath), {
    headers: {
      Authorization: auth
    },
    timeout: 15000,
...
const decodeLyricBytes = _async(function (bytes) {
  if (bytes.length >= 3 && bytes[0] === 0xef && bytes[1] === 0xbb && bytes[2] === 0xbf) {
    return decodeUtf8(bytes.subarray(3)); // UTF-8 BOM
  }
  // 先验 UTF-8，否则尝试 GBK（表不可用则回退 UTF-8）
  return isLikelyUtf8(bytes) ? decodeUtf8(bytes) : _call(loadGbkIndex, function (index) {
...
const loadGbkIndex = _async(function () {
  if (gbkIndex) return gbkIndex;
  if (!gbkIndexPromise) {
    gbkIndexPromise = axios.get(GBK_INDEX_URL, {
      timeout: 20000
    }).then(res => {
      const data = res.data;
...
const loadFileList = _async(function () {
  ...
  const groups = [];
  return _continue(_forOf(getSearchPathList(), function (path) {
    const _push2 = groups.push;
    return _await(scanFolder(client, path), function (_scanFolder3) {
      _push2.call(groups, _scanFolder3);
    });
  }), function () {
...
const scanFolderSlim = function (client, root) {
  return _await(scanFolder(client, root), function (_scanFolder2) {
    return flattenFiles([_scanFolder2]);
  });
};
function _empty() {}
// 递归收集某个目录下的全部音频文件，返回原始文件对象数组。
// 优先用一次深度 PROPFIND（Depth: infinity）；PikPak 不支持/失败时回退为“逐层并发 BFS”。
const scanFolder = _async(function (client, root) {
  let _exit = false;
  // 1) 尝试一次性深度遍历
  return _continue(_catch(function () {
    return _await(client.getDirectoryContents(root, {
      deep: true
    }), function (all) {
      const files = all.filter(isAudio);
      // 深度遍历应返回嵌套文件；若只拿到根目录这一层，则判定为不支持并回退。
      const rootLen = root.replace(/\/+$/, "").length;
      const hasNested = files.some(f => f.filename.replace(/\/+$/, "").lastIndexOf("/") > rootLen);
    ...
    return _continue(_for(function () {
      return !!level.length && visited < MAX_FOLDERS;
    }, void 0, function () {
      const batch = level.slice(0, MAX_FOLDERS - visited);
      visited += batch.length;
      return _await(mapPool(batch, CONCURRENCY, function (path) {
        return _await(_catch(function () {
          return _await(client.getDirectoryContents(path));
        }, function () {
          return [];
        }));
      }), function (listings) {
...
const mapPool = _async(function (items, limit, fn) {
  const results = new Array(items.length);
  let cursor = 0;
  const workers = new Array(Math.min(limit, items.length)).fill(0).map(_async(function () {
    return _continueIgnored(_for(function () {
      return cursor < items.length;
    }, void 0, function () {
      const idx = cursor++;
      return _await(fn(items[idx], idx), function (_fn) {
        results[idx] = _fn;
      });
    }));
  }));
  return _await(Promise.all(workers), function () {
    return results;
  });
...
const GBK_INDEX_URL = "https://raw.githubusercontent.com/cheedonghu/musicfree-plugin/main/gbk-index.json";
const AUDIO_EXTS = [".mp3", ".flac", ".m4a", ".aac", ".wav", ".ogg", ".ape", ".wma", ".opus"];
let cachedData = {};
...
function getClient() {
  ...
  if (cachedData.username !== username || cachedData.password !== password || cachedData.searchPath !== searchPath) {
    cachedData = {
      username,
      password,
      searchPath,
      searchPathList: (searchPath || "").split(",").map(it => it.trim()).filter(Boolean),
      fileList: null,
      lyricCache: {},
      sheetCache: {}
    };
...
function getSearchPathList() {
  return cachedData.searchPathList && cachedData.searchPathList.length ? cachedData.searchPathList : [DEFAULT_PATH];
}
...
function isAudio(file) {
  return file.type === "file" && ((file.mime || "").startsWith("audio") || hasAudioExt(file.basename));
}
...
function toMusicItem(file, order) {
  const {
    artist,
    title
  } = parseName(file.basename, order);
  return {
    id: file.id,
    // 绝对 WebDAV 路径，getMediaSource 据此拼播放地址
    title,
    artist,
    album: "未知专辑"
  };
...
function isLikelyUtf8(bytes) {
  ...
  while (i < n) {
    ...
    let need;
    if (b >= 0xc2 && b <= 0xdf) need = 1;else if (b >= 0xe0 && b <= 0xef) need = 2;else if (b >= 0xf0 && b <= 0xf4) need = 3;else return false;
...
function decodeGbk(bytes, index) {
  ...
  while (i < n) {
    ...
    const b2 = bytes[i + 1];
    const valid2 = b2 !== undefined && (b2 >= 0x40 && b2 <= 0x7e || b2 >= 0x80 && b2 <= 0xfe);
...
module.exports = {
  ...
  getTopLists: _async(function () {
    const client = getClient();
    if (!client) {
      return [];
    }
    const groups = [];
    return _continue(_forOf(getSearchPathList(), function (root) {
      let entries;
      return _continue(_catch(function () {
        return _await(client.getDirectoryContents(root), function (_client$getDirectoryC) {
          entries = _client$getDirectoryC;
        });
      }, function () {}), function () {
  ...
  getTopListDetail: _async(function (topListItem) {
    const client = getClient();
    return client ? _await(scanFolderSlim(client, topListItem.id), function (files) {
      const order = getNameOrder();
      return {
        ...topListItem,
  ...
  getRecommendSheetTags: _async(function () {
    return {
      pinned: [{
        id: "pikpak",
        title: "PikPak"
  ...
  getRecommendSheetsByTag: _async(function () {
    const client = getClient();
    return client ? {
      isEnd: true,
      data: [{
  ...
  getMusicSheetInfo: _async(function (sheetItem, page) {
    ...
    return _invoke(function () {
      if (!page || page <= 1) {
        // 第 1 页：实时重新扫描，反映当前增删
        return _await(buildSheetMusicList(client, sheetId), function (_buildSheetMusicList) {
          list = _buildSheetMusicList;
          cache[sheetId] = list;
          console.log("[PikPak] sheet built", {
            id: sheetId,
            count: list.length
          });
        });
  ...
  importMusicSheet: _async(function (urlLike) {
    const client = getClient();
    if (!client) {
      return null;
    }
    const roots = urlLike && urlLike.trim() ? [urlLike.trim()] : getSearchPathList();
    const groups = [];
    return _continue(_forOf(roots, function (root) {
      const _push = groups.push;
      return _await(scanFolder(client, root), function (_scanFolder) {
        _push.call(groups, _scanFolder);
      });
    }), function () {
      const order = getNameOrder();
